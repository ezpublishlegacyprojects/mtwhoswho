...
{section show=$image_content.is_valid}
    ...
    {section show=and( is_set( $image ), $image )}
        ...
        CAN EDIT="{$attribute.object.can_edit}"
        <img src={$image.url|ezroot} {if $attribute.object.can_edit}id="whoswho_{$attribute.id}"{/if} width="{$image.width}" height="{$image.height}" {section show=$hspace}hspace="{$hspace}"{/section} style="border: {$border_size}px;" alt="{$image.text|wash(xhtml)}" title="{$image.text|wash(xhtml)}" />
		...
		<script type="text/javascript">
			<!--
				params={
					...
					border_color:"#005493",
					type:"nodes",
					url:"{/literal}{$attribute.object.main_node.url_alias|ezurl(no)}{literal}",
				}
			-->
		</script>
	{/literal}
	<div><img src="{'identity.png'|ezimage('no')}" id="whoswho_identity_{$attribute.id}" class="pointer"/></div>
{/if}